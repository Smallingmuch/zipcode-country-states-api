...
/**
 This library allows your to get country code, Country and their respective states
 Set the id of your ZipCode select to "CCode"
 Set the id of your Country select to "country"
 Set the id of your State select to "state"

 then place this code below the country select
 <label class="dynamic_state" id="dynamic_state">State</label>

**/


        var CCode= document.getElementById("CCode");
        function ShowPick(opt){
            //alert(opt);
        }

        $(document).ready(function(){
             for (var i = 0; i < countriesCode.length; i++){
                var optname = countriesCode[i].name;
                var optcode = countriesCode[i].code;
                var el = document.createElement("option");
                el.textContent = optname +" "+optcode;
                el.value=optcode;
                CCode.appendChild(el);
             };
